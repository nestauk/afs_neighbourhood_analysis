...
alt.Chart(diag).mark_point(filled=True).encode(x="pca",y="value",row="diagnostic_var",
                                            color="comm_resolution:O").resolve_scale(y="independent").properties(width=200,height=100)

# %%
pca_mean = diag.groupby(["pca","diagnostic_var"])["value"].median().reset_index(drop=False)

alt.Chart(pca_mean).mark_line(point=True).encode(x="pca",y="value",color="diagnostic_var").properties(width=200,height=100)

# %%
com_res_mean = diag.groupby(["comm_resolution","diagnostic_var"])["value"].median().reset_index(drop=False)

alt.Chart(com_res_mean).mark_line(point=True).encode(x="comm_resolution",y="value",color="diagnostic_var").properties(width=200,height=100)
...
from afs_neighbourhood_analysis.pipeline.lad_clustering.conseq_clustering import most_recent_data
