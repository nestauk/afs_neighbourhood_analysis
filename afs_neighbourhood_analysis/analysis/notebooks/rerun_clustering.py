...
from afs_neighbourhood_analysis.utils.utils import load_colours, nesta_theme
from afs_neighbourhood_analysis.analysis.report_analysis import get_cluster_lookup
...
# %%
#clust = extract_clusters(phf_ahah, 5, 0.5, clustering_params)
...
cluster_table = (
    ...
    .reset_index(name="Top 40 C/UAs by population")
)
...
# %%
#cluster_table.to_markdown(f"{PROJECT_DIR}/outputs/cluster_table_fsm_ahah.md", index=False)
...
# %%
#with open(f"{PROJECT_DIR}/inputs/data/cluster_lookup_ahah_fsm.json", "w") as outfile:
...
#plt.savefig(f"{PROJECT_DIR}/outputs/figures/png/cluster_map_fsm_ahah.svg", format="svg")
...
#save_altair(altair_text_resize(reg_bar), "cluster_nuts_fsm_ahah", driver=wd)
...
save_altair(altair_text_resize(socio), "cluster_demo_ahah_fsm", driver=wd)
...
# %%
phf_long = phf_for_analysis(phf_ahah, clust_lu, code_name_lookup)

# %%
phf_long.rename(columns={"level_1":"indicator_name_expanded"}, inplace=True)
...
cluster_results = (
    ...
    .apply(
        lambda df: ", ".join(
            [
                f"{row['indicator_name_expanded']} ({'+' if row['direction_pos'] else ''}{row['direction']:.2f})"
...
cluster_results.to_markdown(
    f"{PROJECT_DIR}/outputs/secondary_differences_fsm_ahah.md", index=False
)
cluster_results
...
# %%
save_altair(altair_text_resize(ey_perf_2019), "outcomes_2019_fsm_ahah", driver=wd)
...
def plot_outperf(
    ...
    indicator: str = "3. Good Level of Development",
    gender: str = "Total",
    year: int = 2019,
    sd_scale: float = 1.2,
    var_name="score",
    ...
    **kwargs
    ...
    out_under = (
        alt.Chart(ey_renorm)
        .mark_point(filled=True, stroke="black", strokeWidth=0, size=size)
        .encode(  # x=alt.X("cluster:O",scale=alt.Scale(domain=[-1,8])),
            x=alt.X("cluster:O", title="Cluster"),
            # y="score_in_cluster",
            y=alt.Y(
                "score_in_cluster", scale=alt.Scale(type="symlog", domain=[-2.8, 3])
            ),
            color=alt.Color(
                "score_in_cluster", scale=alt.Scale(scheme="spectral"), sort="descending"
            ),
            tooltip=["la_name"],
        )
    ...
    out_under_names = (
        ...
        .encode(
            ...
            y=alt.Y(
                ...
                title="Percentage of children reaching a good level of development (normalised)",
            ),
            # y="score_in_cluster",
            text="outp",
            color=alt.Color(
...
# %%
altair_text_resize(plot_outperf(ey_bench, indicator = "3. Good Level of Development", names=False, size=400))

# %%
save_altair(altair_text_resize(plot_outperf(ey_bench, indicator = "3. Good Level of Development", names=False, size=400)), "outperforming", driver=wd)
...
# %%
def trend_analysis(table, indicator="2. Expected Level of Development", gender="Total"):
...
pipe(
    ...
    altair_text_resize,
    partial(save_altair, name="gender_snapshot_fsm_ahah", driver=wd),
)
...
pairwise_comps_gender = make_pairwise_combs(
    gender_gap, "2. Expected Level of Development", 2019, var_name="ratio"
)

#save_altair(
#    altair_text_resize(make_comp_chart(pairwise_comps_gender, col_thres=0.01)),
#    "gender_difference_comparison_fsm",
#    driver=wd,
#)
...
#    altair_text_resize,
#    partial(save_altair, name="gender_trend_fsm", driver=wd),
#)
